...
& cmake "-DBACKEND_DLL=$backend" "-DROCM_ROOT=$rocm" "-DDESTINATION=$stage" -P $collector
...
if (-not $rocblas) { throw "rocBLAS kernel directory was not found below '$rocm'." }
...
$licenseFiles = Get-ChildItem -LiteralPath $rocm -Recurse -File -ErrorAction SilentlyContinue |
...
foreach ($license in $licenseFiles) {
    $relative = [IO.Path]::GetRelativePath($rocm, $license.FullName)
